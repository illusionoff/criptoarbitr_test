...
function wsGetGate(time, channel, event, payload, initialBith, writableFiles) {
  ...
  ws.onmessage = function (evt) {
    ...
    timeNaw = Number(initialGate.messageObj.time);
    colMessage++;
    ...
    console.log(`GATE maxTimePeriod=${maxTimePeriod}, colMessage=${colMessage}, timeNaw=${timeNaw}, time All=${timeAll}`);
    ...
    if (initialGate.messageObj.event == "update" && initialGate.messageObj.result.bids) {
      ...
      console.log('initialGate.messageObj.result.bids=', initialGate.messageObj.result.bids);
      console.log('222 initialGate.bay=', initialGate.bay);
      console.log('initialGate.messageObj.result.bids=', initialGate.messageObj.result.asks);
      console.log('222 initialGate.bay=', initialGate.sell);
      ...
      if (initialBith.initialFetchURL && initialBith.initialWs) { // если готовы данные из bithumb
        ...
        let strResultPrevious = String(initialGate.orderbookFirstPreviousBay) +
          String(initialGate.orderbookFirstPreviousSell);
        let strResultNaw = String(initialGate.bay) + String(initialGate.sell);
        console.log('changeTradeArr strResultPrevious =', strResultPrevious);
        console.log('changeTradeArr strResultNaw =', strResultNaw);
        console.log(' changeTradeArr typeof initialGate.orderbookFirstPreviousBay=', typeof initialGate.orderbookFirstPreviousBay);
        console.log(' changeTradeArr typeof initialGate.orderbookFirstPreviousSell=', typeof initialGate.orderbookFirstPreviousSell);
        console.log(' changeTradeArr typeof initialGate.bay=', typeof initialGate.bay);
        console.log(' changeTradeArr typeof initialGate.sell=', typeof initialGate.sell);;



        if (changeTradeArr(initialGate)) {

          console.log('Gate changeTradeArr= true');
          // process.exit();
          ...
          console.log('strResultPrevious =', strResultPrevious);
          console.log('strResultNaw =', strResultNaw);

          if (strResultPrevious === strResultNaw) {
            console.log('strResultPrevious === strResultNaw');
            process.exit();
          }

          goTrade(paramsGoTrade, writableFiles);
        } else {
          console.log('Gate changeTradeArr= false');
