  ...
  ws.onmessage = function (evt) {
    ...
    if (initialGate.messageObj.params) {
      ...
      if (initialGate.messageObj.params[1].bids && initialGate.messageObj.params[1].bids.length != 0) {
        initialGate.priceBay = Number(initialGate.messageObj.params[1].bids[0][0]);
      }
      if (initialGate.messageObj.params[1].asks && initialGate.messageObj.params[1].asks.length != 0) {
        initialGate.priceSell = Number(initialGate.messageObj.params[1].asks[0][0]);
      }
      ...
      // Если будет один элемент и его количество было 0, то возратится пустой массив - это сломает программу. Организовать проверку на количество или пустой массив
      if (initialGate.messageObj.params[1].asks && (initialGate.messageObj.params[1].asks.length != 0) && (initialGate.messageObj.params[1].bids.length != 0) &&
        initialGate.messageObj.params[1].bids && initialGate.priceSell != undefined && initialGate.priceSell > 0 && initialGate.priceBay != undefined &&
        initialGate.priceBay > 0) initialGate.globalFlag = true;
