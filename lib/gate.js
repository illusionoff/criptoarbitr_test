...
const WS = require('ws');
const { goTrade, changeTradeArr, correctTimeServerClosure, averageTimeDiffMessageClosure } = require('../functions/functions');
...
function wsGetGate(time, channel, event, payload, initialBith, writableFiles) {
  ...
  function startPing(time) {
    ping = setInterval(function () {
      console.log('ping==========================================================================');
      // '{"time": %d, "channel" : "spot.ping"}' % int(time.time())
      // ws.send(JSON.stringify({ `"time": ${new Date().getTime()}` }));
  ...
  let averageTimeDiffMessage = averageTimeDiffMessageClosure();
  // function testOnce() {
  //   console.log('testOnce WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW');
  //   let time = new Date().getTime();
  //   console.log('time testOnce=', time);
  //   return time
  // }
  // let canOnlyFireOnce = once(testOnce);
  ...
  let startPingOnce = once(startPing, 10000);
  let correctTimeServer = correctTimeServerClosure();//, ws, colMessage
  ...
  ws.onmessage = function (evt) {
    ...
    timeNaw = Math.round((new Date().getTime()) / 1000);
    ...
    timeAll = timeNaw - timeStart;
    let viewMAxTimePeriod = Math.round((maxTimePeriod) / 1000);
    console.log(` GATE viewMAxTimePeriod=${viewMAxTimePeriod}, colMessage=${colMessage}, timeNaw=${timeNaw}, time All=${timeAll}`);
    ...
    if (initialGate.messageObj.event == "update" && initialGate.messageObj.result.bids) {
      ...
      console.log('initialGate.timeServer=', initialGate.timeServer);
      console.log(' initialGate.timeGate=', initialGate.timeGate);
      if (initialGate.globalFlag) {
        testTimeArr.push([initialGate.timeServer, initialGate.timeGate]);
        tesTimeCount++;
        if (tesTimeCount === 20) {
          console.log('Test Time Gate');
      ...
      console.log('length=', initialGate.messageObj.result.bids.length);
      const length = initialGate.messageObj.result.bids.length - 1;
      console.log('bids[0][0]=', initialGate.messageObj.result.bids[0][0]);
      console.log('111 bids[length][0]=', initialGate.messageObj.result.bids[length][0]);
      ...
      console.log('percent bids[0][0]-bids[length][0]=', percent);
      if (percent > maxPercent) {
        maxPercent = percent;
      }
      console.log('maxPercent=', maxPercent); //  за 5 минут получил 0.109 %
      // берем самый худший результат т.е  последний элемент массива
      initialGate.bay = Number(initialGate.messageObj.result.bids[length][0]);
      initialGate.sell = Number(initialGate.messageObj.result.asks[length][0]);
      // initialGate.globalFlag = true;
      initialBith.initialFetchURL = initialBith.initialWs = true;
      if (initialGate.globalFlag && initialBith.initialFetchURL && initialBith.initialWs) { // если готовы данные из bithumb
        if (changeTradeArr(initialGate)) {
          const paramsGoTrade = {
            ...
            timeServer: initialGate.timeServer - initialGate.timeFileServerCorrect,
    ...
    let resultCorrectTimeServer = correctTimeServer(ws, initialGate);
    console.log('resultCorrectTimeServer1=', resultCorrectTimeServer);

    if (resultCorrectTimeServer && typeof resultCorrectTimeServer === "number") {
      console.log('resultCorrectTimeServer2=', resultCorrectTimeServer);
      startPingOnce(10000);
      let resultAverageTimeDiff = averageTimeDiffMessage(initialGate);
      if (resultAverageTimeDiff) {
        console.log('resultAverageTimeDiff=', resultAverageTimeDiff);
        // let timeFileServer = initialGate.timeServer - resultAverageTimeDiff + resultCorrectTimeServer;
        // суммарное время коррекции
        let timeFileServerCorrect = resultAverageTimeDiff - resultCorrectTimeServer - config.get("TIME_DALAY");
        console.log('timeFileServerCorrect=', timeFileServerCorrect);
        console.log('initialGate.timeServer=', initialGate.timeServer);
        console.log('initialGate.timeGate=', initialGate.timeGate);

        console.log('initialGate.timeFileServerCorrect=', initialGate.timeFileServerCorrect);

        initialGate.timeFileServerCorrect = timeFileServerCorrect;
        initialGate.globalFlag = true;
        // process.exit();
      } else {
    ...
    if (!resultCorrectTimeServer) {
      console.log('resultCorrectTimeServer3=', resultCorrectTimeServer);
      // process.exit();
      return ws.reconnect(1006, 'Reconnect error');
    }
