...
const WS = require('ws');
const { goTrade, changeTradeArr, reconnectTimeMessageClosure } = require('../functions/functions');
...
function wsGetGate(time, channel, event, payload, initialBith, writableFiles) {
  ...
  ws.onmessage = function (evt) {
    ...
    console.log(`countReconnect= ${countReconnect}, countErrors= ${countErrors}`);
    ...
    if (initialGate.messageObj.event == "update" && initialGate.messageObj.result.bids) {
      ...
      if (!Boolean(initialGate.orderbookFirstPreviousBay)) {
        initialGate.orderbookFirstPreviousBay = initialGate.bay;
      }
      if (!Boolean(initialGate.orderbookFirstPreviousSell)) {
        initialGate.orderbookFirstPreviousSell = initialGate.sell;
      }
