...
function wsGetGate(time, channel, event, payload, initialBith, writableFiles) {
  ...
  ws.onmessage = function (evt) {
    ...
    if (initialGate.messageObj.error) {
      console.log('Reconnect eroro ping', initialGate.messageObj.error);
    ...
    if (initialGate.messageObj.event == "update" && initialGate.messageObj.result.bids) {
      ...
      console.log('bids[length][0]=', initialGate.messageObj.result.bids[length][0]);
      ...
      initialGate.bay = initialGate.messageObj.result.bids[length][0];
      initialGate.sell = initialGate.messageObj.result.asks[length][0];
      //  Инициализация первых предыдущих значений
      if (!orderbookFirstPreviousBay) {
        orderbookFirstPreviousBay = initialGate.bay;
      }
      if (!orderbookFirstPreviousSell) {
        orderbookFirstPreviousSell = initialGate.sell;
      }

      // проверка изменения значения для предотвращения лишних вычислений
      if (orderbookFirstPreviousBay && initialGate.bay != orderbookFirstPreviousBay) {
        orderbookFirstPreviousBay = initialGate.bay;
      ...
      if (orderbookFirstPreviousSell && initialGate.sell != orderbookFirstPreviousSell) {
        orderbookFirstPreviousSell = initialGate.sell;
