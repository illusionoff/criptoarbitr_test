...
let testWriteableStream_2 = undefined;
...
function TestWritable(testWriteableStream_1, testWriteableStream_2, testFlag, testCount, testCountAll) {
  ...
  if (testCount.number > 10) {
    ...
    if (testFlag.number === 1) {
      ...
      testWriteableStream_2 = fs.createWriteStream(`logs/test_profit${time}.csv`, { flags: 'a' });
    ...
    let time = new Date().getTime();
    console.log('time:', time);
    testWriteableStream_1 = fs.createWriteStream(`logs/test_profit${time}.csv`, { flags: 'a' });
  };
  console.log(`testFlag.number=${testFlag.number},----------------------------------------------------------------------------------------------------`);

  if (testFlag.number === 1) {
    testWriteableStream_1.write(`writeableStream_${testCount.number}\r\n`);
  }
  if (testFlag.number === 2) {
    testWriteableStream_2.write(`writeableStream_${testCount.number}\r\n`);
  }
...
function wsStartBith(cmd, args, initialGate, counts, writeableStream) {
  ...
  ws.onmessage = function (message) {
    for (let i = 0; i < 30; i++) {
      TestWritable(testWriteableStream_1, testWriteableStream_2, testFlag, testCount);
    }
