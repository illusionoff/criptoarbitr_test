...
setInterval(() => {
  ...
  initialBith.indexLeveragesOrderbookBay.forEach((value, index) => fileBay.write(`${index}:${value}\r\n`));
  fileBay.end();
  let fileSell = fs.createWriteStream('indexLeveragesOrderbookSell.txt');
  fileSell.on('error', function (err) { Console.log(err) });
  initialBith.indexLeveragesOrderbookSell.forEach((value, index) => fileSell.write(`${index}:${value}\r\n`));
  fileSell.end();
...
function wsStartBith(cmd, args, initialGate, writableFiles) {
  ...
  ws.onmessage = function (message) {
    ...
    if (initialBith.messageObj.code && initialBith.messageObj.code === '0') ws.reconnect(1006, 'initialBith.messageObj.code === 0');
