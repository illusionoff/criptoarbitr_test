    ...
    if (initialBith.messageObj.code === "00007") {
      ...
      if (initialGate.globalFlag) {
        const paramsGoTrade = {
          ...
          timeServer: new Date(),
          timeBith: initialBith.messageEdit.timestamp,
          init: true
        }
        // goTrade(paramsGoTrade);
      };

      if (initialBith.messageObj.data.b.length > 0 && initialBith.messageObj.data.b.length > 0) { // если массив существует и заполнен, то считаем норм
