...
const fs = require("fs");
const { goTrade, reconnectBithClosure } = require('../functions/functions');//correctTimeServerClosure
...
// test
const MINTIME_ONE_PING = 400;
const MINTIME_ALL_PING = 4000;
...
function wsStartBith(cmd, args, initialGate, writableFiles) {
  ...
  function correctTimeServerClosure(colMessage) {
    ...
    function timeServer(colMessage) {
      ...
      console.log('initialBith=', initialBith);

      console.log('code00001=', code00001);
      console.log('colMessage222222222222222222222222222222222222222222222222222222222=', colMessage);
      ...
      if (code00001 === true) {
        ...
        if (count < 12) {
          ...
          if (initialBith.messageObj.code && initialBith.messageObj.code === '0' &&
            ...
            // if ((timePong - timePing) > 200) {
            //   console.log(`(timePong - timePing) > 300ms`);// пришел ответ Pong
            ...
            if (arrTimesPingPong.length > 2) {
              ...
              if (timeDelta > MINTIME_ONE_PING) {
                console.log(`(timePong - timePing) > 200ms timeDelta =${timeDelta}`);// пришел ответ Pong
                // обнуление переменных
                timeStart = undefined;
                count = 0;
                arrTimesPingPong = [];
      ...
      if (timeStart !== undefined && (timeEnd - timeStart) > MINTIME_ALL_PING) {
