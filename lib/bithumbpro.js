...
let testWriteableStream_3 = fs.createWriteStream(`logs/test_profit_3.csv`, { flags: 'a' });
testWriteableStream_3.write(`writeableStream_weeqwewqe\r\n`);
// testWriteableStream_3.destroy(console.log(`testWriteableStream_3.destroy()`));
// testWriteableStream_3 = fs.createWriteStream(`logs/test_profit_4.csv`, { flags: 'a' });
// console.log('testWriteableStream_3._writableState:', testWriteableStream_3._writableState);
// testWriteableStream_3.write(`writeableStream_3_TTTT}\r\n`);
function TestWritable3() {
  let testFlag = { number: 1 };
  // let testCount = { number: 0 };
  let testCount = 0;
  let count = 0;
  let testWriteableStream_10 = {
    write_2: fs.createWriteStream("logs/test_profit_5.csv", { flags: 'a' })
  }

  console.log('count-----------------------------------------------------------------------------', count);
  return function () {
    return count++, testWriteableStream_10.write_2.write(`writeableStream_${count}\r\n`); // есть доступ к внешней переменной "count"
  };
}

// let variableTestWritable3 = TestWritable3();
// // variableTestWritable3();
// // variableTestWritable3();
// // variableTestWritable3();
// // console.log(variableTestWritable3());
// // console.log(variableTestWritable3());

// setInterval(variableTestWritable3, 1000);


// let something = { other: 'other 1' };
// function makeCounter(something) {
//   let count = 0;
...
// let counter = makeCounter(something);
...
function TestWritable2() {
  ...
  function testIf() {
    ...
    if (testCount === 2) {
      ...
      testWriteableStream_2.write_2.write(`writeableStream_${testCount}\r\n`);
    }
...
// let VariableTestWritable2 = TestWritable2(testWriteableStream_1, testWriteableStream_2, testFlag, testCount, testCountAll)

// VariableTestWritable2();
function TestWritable(testWriteableStream_1, testWriteableStream_2) {
  let testFlag = { number: 1 };
  let testCount = { number: 0 };
  let testCountAll = { number: 0 };
  ...
  //   }
  // }
  // if ((testFlag.number === 1) && (testWriteableStream_2)) {
  ...
  if (testCount.number > 20) {
    ...
    if (testFlag.number === 1) {
      ...
      console.log('testWriteableStream_2._writableState:', testWriteableStream_2.write_2._writableState);
      testWriteableStream_1.write_1.on('drain', () => {
        console.log('estWriteableStream_1 drain buffer = 0-------------------------------------------------------------------------------');
        testWriteableStream_1.write_1.end();
      })
      testWriteableStream_1.write_1.on('finish', () => {
        console.log('estWriteableStream_1 The end-------------------------------------------------------------------------------');
        testWriteableStream_1.write_1.close();
        // testWriteableStream_1.destroy();
      })
      return
    }
    ...
    let time = new Date().getTime();
    console.log('time:', time);
    testWriteableStream_1.write_1 = fs.createWriteStream(`logs/test1_profit${time}.csv`, { flags: 'a' });

    console.log('testWriteableStream_1._writableState:', testWriteableStream_1.write_1._writableState);
    testWriteableStream_2.write_2.on('drain', () => {
      console.log('estWriteableStream_1 drain buffer = 0-------------------------------------------------------------------------------');
      testWriteableStream_2.write_2.end();
    })
    testWriteableStream_2.write_2.on('finish', () => {
      console.log('estWriteableStream_2 The end-------------------------------------------------------------------------------');
      testWriteableStream_2.write_2.close();
    })
  };
  ...
  if (testFlag.number === 1) {
    ...
    testWriteableStream_1.write_1.write(`writeableStream_${testCountAll.number}\r\n`);
    console.log('wtiten_1=---------------------------------------------------------------------');
  }
  if (testFlag.number === 2) {
    console.log('writeableStream_2');
    console.log('testWriteableStream_2._writableState:', testWriteableStream_2.write_2._writableState);
    // console.log('testWriteableStream_2._writableState.onwrite :', testWriteableStream_2._writableState.onwrite);
    testWriteableStream_2.write_2.write(`writeableStream_${testCountAll.number}\r\n`);
  }
  // testWriteableStream_1.write(`writeableStream_${testCount}\r\n`);
...
function wsStartBith(cmd, args, initialGate, counts, writeableStream) {
  // let variableWritable = TestWritable(testWriteableStream_1, testWriteableStream_2);
  ...
  writeableStream.on('error', function () {
    console.log('writeableStream ERROR 1-----------------------------------------------:');
    process.exit();
  ...
  ws.onmessage = function (message) {
    ...
    if (initialBith.messageObj.code === "00007") {
      ...
      if (initialGate.globalFlag) {
        ...
        // goTrade(paramsGoTrade, counts, writeableStream);
      };

      if (initialBith.messageObj.data.s.length > 0 && initialBith.messageObj.data.s.length > 0) { // если массив существует и заполнен, то считаем норм
        console.log('initialBith.messageObj.data.b.length > 0 && initialBith.messageObj.data.b.length > 0');
        initialBith.initialWs = true;
      }
