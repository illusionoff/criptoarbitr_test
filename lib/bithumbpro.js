...
const fs = require("fs");
const { goTrade, reconnectBithClosure, correctTimeServerClosure } = require('../functions/functions');//correctTimeServerClosure
...
function orderbookChange(allOrderbook, newMessage, BayOrSell) {
  ...
  allOrderbook.sort((b, a) => { return Number(b[0]) - Number(a[0]) });
  console.log('!!! allOrderbook =  ***************************', BayOrSell);
  for (let i = 0; i < 5; i++) {
    console.log(allOrderbook[i]);
    // console.log(allOrderbookSell[i]);
  }

  if (!BayOrSell) process.exit();
...
function wsStartBith(cmd, args, initialGate, writableFiles) {
  ...
  ws.onmessage = function (message) {
    ...
    let resultCorrectTimeServer = correctTimeServer(ws, initialBith);
