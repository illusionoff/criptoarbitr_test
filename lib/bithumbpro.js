  ...
  ws.onmessage = function (message) {
    ...
    if (initialBith.messageObj.code === "00007") {
      if (Number(initialBith.messageObj.data.ver) === initialBith.ver + 1) {
        ...
        if (initialBith.messageObj.data.b.length = 1) {
          const index = indexOfTwoDimens(allOrderbookBay, initialBith.messageObj.data.b[0][0]);
          if (index) {
            if (initialBith.messageObj.data.b[0][1] === '0.000000') {
              //  удалить из массива этот элемент
              allOrderbookBay.splice(index, 1);
            } else {
              // заменить новым значением
              allOrderbookBay[index][1] = initialBith.messageObj.data.b[0][1];
            }

          } else {
            // // Если элемент не найден, то добавть в массив и упорядочить по убыванию
            allOrderbookBay.push(initialBith.messageObj.data.b).sort((a, b) => (b - a));
          }
        };
