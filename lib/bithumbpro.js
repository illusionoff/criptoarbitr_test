...
const fs = require("fs");
const { goTrade, reconnectTimeMessageClosure } = require('../functions/functions');
...
const TRACK_ELEMENT_ORDERBOOK = 10;
  ...
  ws.onmessage = function (message) {
    ...
    if (initialBith.messageObj.code === "00007") {
      ...
      if (Number(initialBith.messageObj.data.ver) === initialBith.ver + 1) {
        ...
        const trackElementOrderbook0 = allOrderbookBay[TRACK_ELEMENT_ORDERBOOK][0];
        const trackElementOrderbook1 = allOrderbookBay[TRACK_ELEMENT_ORDERBOOK][1];

        // console.log('changeFirstOrderbook=', changeFirstOrderbook(orderbookFirstPreviousBay, allOrderbookBay));
        if (orderbookFirstPreviousBay[0] == trackElementOrderbook0 && orderbookFirstPreviousBay[1] == trackElementOrderbook1) {
          console.log('orderbookFirstPreviousBay==========================================', orderbookFirstPreviousBay);
          console.log('allOrderbookBay[TRACK_ELEMENT_ORDERBOOK]========================', allOrderbookBay[TRACK_ELEMENT_ORDERBOOK]);
        ...
        if (changeFirstOrderbook(orderbookFirstPreviousBay, allOrderbookBay[TRACK_ELEMENT_ORDERBOOK])) {
          initialBith.bay = Number(trackElementOrderbook0);
          // не реализовано changeFirstOrderbook(orderbookFirstPreviousSell, allOrderbookSell)
          initialBith.sell = Number(trackElementOrderbook0);

          // initialBith.bayTimestamp = initialBith.messageObj.timestamp;
          initialBith.bayQuantity = Number(trackElementOrderbook1);
