...
function reconnectBithClosure() {
  ...
  function startReconnect() {
    ...
    console.log('startReconnect count=', count);
    if (count > 9) {
      let timeDiff = timeNaw - timePrevious;
      console.log('timeDiff=', timeDiff);
      if (timeDiff > 20000) {
        console.log('Reconnect Bith');
        count = 0;
...
function wsStartBithOrder10(cmd, args, initialGate, writableFiles) {
  ...
  ws.onmessage = function (message) {
    ...
    if (initialBith.messageObj.code && initialBith.messageObj.code === '0' &&
      ...
      if (viewMAxTimePeriod >= 5) {
        console.log('reconnect!!! viewMAxTimePeriod>= 20');
        return ws.reconnect(1006, 'viewMAxTimePeriod>= 20');
      }
